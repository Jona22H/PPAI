...
class Bodega {
    ...
    crearVino(vinoACrear) {
        ...
        let vinoNuevo = new vino_1.default(vinoACrear.getNombre(), this, vinoACrear.getAñada(), new Date(), vinoACrear.getImagenEtiqueta(), vinoACrear.getNotaCata(), vinoACrear.getPrecio(), vinoACrear.getReseña(), tiposUvas, maridaje);
        return vinoNuevo;
    }
    buscarMaridaje(vinoACrear) {
        ...
        maridajesVinoACrear.forEach((maridajeAAsignar) => {
            for (let i = 0; data_1.dataMaridajes.length > i; i++) {
                let maridajeEnBd = data_1.dataMaridajes[i];
    ...
    actualizarVinos(vinosAActualizar) {
        ...
        for (let i = 0; data_1.dataVinoEnBD.length > i; i++) {
            let vino = data_1.dataVinoEnBD[i];
        ...
        vinosAActualizar.forEach((vinoACrear) => {
            ...
            data_1.dataVinoEnBD.push(vinoNuevo);
        });
