...
var PantallaAdministradorActualizacionBonVino = /** @class */ (function () {
    function PantallaAdministradorActualizacionBonVino() {
    }
    PantallaAdministradorActualizacionBonVino.prototype.opcionImportarActualizacion = function (gestor) {
        this.mostrarPantalla(gestor);
        this.gestor = gestor;
    };
    PantallaAdministradorActualizacionBonVino.prototype.mostrarPantalla = function (gestor) {
        gestor.opcionImportarActualizacion(this);
    };
    PantallaAdministradorActualizacionBonVino.prototype.mostrarBodegasConActu = function (bodegasConActualizaciones) {
        ...
        if (bodegasConActualizaciones.length > 0) {
            ...
            for (var i = 0; bodegasConActualizaciones.length > i; i++) {
                ...
                var opcion = "<div class=\"col-3 \">\n            <div class=\"form-check form-check-inline \">\n                <input class=\"form-check-input fs-3 \" type=\"radio\" id=\"inlineCheckbox".concat(i + 1, "\" value=\"").concat(bodega.getNombre(), "\"\n                    name=\"bodegaAActualizar\">\n                <label class=\"form-check-label fs-3\" for=\"inlineCheckbox1\">").concat(bodega.getNombre(), "</label>\n            </div>\n        </div>");
    ...
    PantallaAdministradorActualizacionBonVino.prototype.mostrarResumenDeActualizacion = function (vinosAMostrar) {
        ...
        for (var _i = 0, vinosAMostrar_1 = vinosAMostrar; _i < vinosAMostrar_1.length; _i++) {
            var vino = vinosAMostrar_1[_i];
            var maridajesAMostrar = vino.vinoAMostrar
                .getMaridaje()
                .map(function (maridaje) { return maridaje.getNombre(); });
            var fila = "<tr>\n        <th scope=\"row\">".concat(vino.vinoAMostrar.getBodega().getNombre(), "</th>\n        <td>").concat(vino.vinoAMostrar.getNombre(), "</td>\n        <td>").concat(vino.vinoAMostrar.getAñada(), "</td>\n        <td>").concat(vino.vinoAMostrar.getFechaActualizacion(), "</td>\n        <td>").concat(vino.vinoAMostrar.getImagenEtiqueta(), "</td>\n        <td>").concat(vino.vinoAMostrar.getNotaCata(), "</td>\n        <td>$").concat(vino.vinoAMostrar.getPrecio(), "</td>\n        <td>").concat(vino.vinoAMostrar.calcularRanking(), "</td>\n        <td>").concat(vino.varietalesAMostrar.join('\n'), "</td>\n        <td>").concat(maridajesAMostrar, "</td>\n        <td>").concat(vino.estado, "</td>");
            var filaNueva = cuerpoTabla.insertRow();
            filaNueva.innerHTML = fila;
        }
    };
    PantallaAdministradorActualizacionBonVino.prototype.tomarSeleccionBodega = function (fieldSet, bodegasConActualizaciones) {
        var _this = this;
        ...
        boton.addEventListener('click', function () {
            ...
            if (bodegaSeleccionada) {
                var bodegaAEnviar = bodegasConActualizaciones.find(function (bodega) {
                    return bodega.getNombre().toLowerCase() ===
    ...
    return PantallaAdministradorActualizacionBonVino;
}());
export default PantallaAdministradorActualizacionBonVino;
